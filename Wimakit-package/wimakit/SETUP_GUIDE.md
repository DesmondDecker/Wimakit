...
7. Copy the connection string — it looks like:
   ```
   mongodb+srv://desmonddecker684_db_user:<password>@wimakit.tvwhlty.mongodb.net/?appName=Wimakit
   ```
   Replace `<password>` with your actual password (`Didi982003`)
...
MONGODB_URI=mongodb+srv://desmonddecker684_db_user:Didi982003@wimakit.tvwhlty.mongodb.net/?appName=Wimakit
...
CLOUDINARY_CLOUD_NAME=dveodsdcf
CLOUDINARY_API_KEY=272631326253235
CLOUDINARY_API_SECRET=PZ4q_yFgk_ykSuBB6IbJzIo_yOk
...
1. **Push to GitHub:** Make sure your project is on GitHub.
   ...
   git init
   git add .
   git commit -m "Initial WimaKit project"
   # Create a new repository on github.com, then:
   git remote add origin https://github.com/DesmondDecker/Wimakit.git
   # Ensure your primary branch is called 'main' (older Git versions default to 'master')
   git branch -M main
   git push -u origin main
   # If the push is rejected, you may need to force it (this will overwrite the remote):
   # git push -u origin main --force
   ```
2. Go to https://dashboard.render.com → sign up with GitHub.
3. Click **New** → **Blueprint**.
4. Connect the GitHub repository you just created. Render will automatically detect `render.yaml`.
5. Click **Apply**. Render will create two services: `wimakit-api` and `wimakit-web`.
6. Go to the `wimakit-api` service's **Environment** tab.
7. Add your secrets:
   - `MONGODB_URI`
   - `CLOUDINARY_CLOUD_NAME`
   - `CLOUDINARY_API_KEY`
   - `CLOUDINARY_API_SECRET`
8. A new deployment will trigger automatically. Your API will be live at a URL like `https://wimakit-api.onrender.com`.
...
2. Sign up and login:
   ...
   flyctl auth login
   ```
3. **Deploy Backend:** In the `wimakit-backend` folder:
   ```bash
...
4. **Set Backend Secrets:**
   ```bash
   flyctl secrets set MONGODB_URI="mongodb+srv://desmonddecker684_db_user:Didi982003@wimakit.tvwhlty.mongodb.net/?appName=Wimakit"
   flyctl secrets set JWT_SECRET="your_jwt_secret_here"
   flyctl secrets set JWT_REFRESH_SECRET="your_refresh_secret_here"
   flyctl secrets set CLOUDINARY_CLOUD_NAME="dveodsdcf"
   flyctl secrets set CLOUDINARY_API_KEY="272631326253235"
   flyctl secrets set CLOUDINARY_API_SECRET="PZ4q_yFgk_ykSuBB6IbJzIo_yOk"
...
5. **Deploy Backend:**
   ...
   flyctl deploy
   ```
6. **Deploy Frontend:** In the `wimakit` folder (the frontend):
   ```bash
...
✅ Created admin: admin@wimakit.com
✅ Created seller: seller@wimakit.com
✅ Created buyer: buyer@wimakit.com
✅ Created rider: rider@wimakit.com

🎉 Seed complete!
───────────────────────────────────────
  Admin  → admin@wimakit.com   / Admin@123!
  Seller → seller@wimakit.com  / Seller@123!
  Buyer  → buyer@wimakit.com   / Buyer@123!
  Rider  → rider@wimakit.com   / Rider@123!
───────────────────────────────────────
✅ Default delivery config seeded
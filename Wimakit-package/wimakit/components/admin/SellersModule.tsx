...
function useTrendStore() {
  const qc = useQueryClient();
  return useMutation({
    // Previously this only ever took an id and called adminApi.trendStore(id),
    // which always meant "mark trending" — there was no way for the caller
    // (handleTrend below) to say "un-trend this one". Accepting the target
    // state here is what actually lets the Remove-Trend action work.
    mutationFn: ({ id, isTrending }: { id: string; isTrending: boolean }) =>
      adminApi.trendStore(id, isTrending).then(r => r.data),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['admin', 'sellers'] }),
  });
}
    ...
    Alert.alert(
      ...
      [
        ...
        { text: isTrending ? 'Remove' : 'Mark Trending 🔥', onPress: () =>
          trendMut.mutate({ id: seller._id ?? seller.id, isTrending: !isTrending }, {

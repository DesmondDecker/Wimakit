...
const PostCard = memo(({ post, onPress, onComment, onShare, onReport, onDelete, onBlock, compact = false }: Props) => {
  ...
  const handleReact = useCallback((type: string) => {
    ...
    if (!uid) {
      Toast.show({ type: 'info', text1: 'Please sign in to react to posts' });
      return;
    }
    if (!pid) {
      Toast.show({ type: 'error', text1: 'Invalid post id' });
      return;
    }

    reactMut.mutate({ id: pid, type }, {
      onSuccess: (data: any) => {
        if (data?.queued) Toast.show({ type: 'info', text1: "Saved — will send once you're back online" });
      },
      onError: (err: any) => {
        const msg = err?.response?.data?.message ?? err?.message ?? 'Failed to react';
    ...
    >
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity
          style={s.moreBtn}
          onPress={() => {
            // Settings > Blocked Users previously had no way to actually
            // block anyone from anywhere in the app — this is the entry
            // point. Only offered on someone else's post, obviously.
            const opts = ['Report'];
            if (!isOwn && onBlock) opts.push('Block User');
            if (isOwn) opts.unshift('Delete Post');
            const buttons: any[] = opts.map(o => ({
              text: o,
              style: (o === 'Delete Post' || o === 'Block User') ? 'destructive' : 'default',
              onPress: o === 'Delete Post' ? onDelete : o === 'Block User' ? onBlock : onReport,
            }));
      ...
      <View style={[s.actions, { borderTopColor: colors.border }]}>
        ...
        <TouchableOpacity
          ...
          onPress={onComment ?? (() => {
            const pid = post._id ?? post.id;
            if (pid) router.push(`/community/post/${pid}` as any);
          })}
        >

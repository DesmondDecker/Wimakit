...
export const ProductCard = memo(({ product, horizontal = false, index = 0 }: ProductCardProps) => {
  ...
  // listProducts (backend) doesn't filter out products with stock: 0 — only
  // isAvailable/status — so a sold-out product still reaches this card with
  // its Add/Buy buttons rendered live. Tapping them called addItem(product,
  // 1), and the store's own Math.min(qty, stock) ceiling silently clamped
  // that to a quantity-0 line: no error, nothing visibly changed, cart
  // count didn't move. That is what "Buy button does nothing" looks like
  // from the outside. useCartStore now refuses a stock:0 add on its own as
  // a backstop, but guarding here too means the button honestly reflects
  // that it won't do anything, instead of looking live and silently
  // failing.
  ...
  const onAddToCart = (e: any) => {
    e.stopPropagation();
    if (outOfStock) {
      Toast.show({ type: 'info', text1: 'Out of stock' });
      return;
    }
    addItem(product, 1);
  ...
  const imageUri = normalizeImageUri(product.images?.[0]);
  ...
  if (horizontal) {
    return (
      <TouchableOpacity 
        ...
        activeOpacity={0.85}
      >
        <Image source={{ uri: imageUri }} style={styles.imageHoriz} contentFit="cover" transition={200} />
        <View style={styles.contentHoriz}>
          ...
          <TouchableOpacity 
            style={[styles.buyBtnSmall, { backgroundColor: outOfStock ? colors.border : colors.primary }]} 
            onPress={onBuyNow}
            disabled={outOfStock}
          >
            <Text style={[styles.buyBtnTextSmall, outOfStock && { color: colors.textMuted }]}>{outOfStock ? 'Sold Out' : 'Buy Now'}</Text>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
    );
      ...
      >
        ...
        <View style={styles.content}>
          ...
          <View style={styles.cardActions}>
            <TouchableOpacity 
              style={[styles.actionBtn, { backgroundColor: colors.surfaceRaised, borderColor: colors.border + '30', opacity: outOfStock ? 0.5 : 1 }]} 
              onPress={onAddToCart}
              disabled={outOfStock}
            >
            ...
            <TouchableOpacity 
              style={[styles.actionBtn, { backgroundColor: outOfStock ? colors.border : colors.primary }]} 
              onPress={onBuyNow}
              disabled={outOfStock}
            >
              <Text style={[styles.actionBtnText, { color: outOfStock ? colors.textMuted : '#fff' }]}>{outOfStock ? 'Sold Out' : 'Buy'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </TouchableOpacity>

...
import { normalizeImageUri } from '../utils/api';
...
import { ReceiptInput } from '../utils/receipt';
import { ReceiptView } from '../components/order/ReceiptView';
import { Button } from '../components/ui/Button';
import { Spacing, Radius, FontSize } from '../constants/theme';
import { PlatformMap, PlatformMapMarker, PlatformMapUrlTile } from '../components/PlatformMap';
type Step = 'cart' | 'checkout' | 'paying' | 'confirmed';
...
export default function CartScreen() {
  ...
  const [receiptData,     setReceiptData]     = useState<ReceiptInput | null>(null);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (isDialCodeMethod) {
        // Mobile money still needs the USSD dial-instructions screen before
        // the buyer has actually paid — the receipt is available afterward
        // from the order tracking page (see app/order/[id].tsx), same as
        // every other payment method.
        clearCart(); setPlacing(false); setStep('paying');
      } else {
        // cod / wallet / bnpl are all fully settled (or scheduled) the
        // moment the order exists — show the same confirmation + receipt
        // screen for all three instead of singling out COD. Seller isn't
        // populated on the order-creation response (see orderController's
        // createOrder, which doesn't populate() it), so pull seller details
        // from the cart's product data, which already carries them.
        const sellerPhone = (items[0]?.product?.seller as any)?.phone;
        const receipt: ReceiptInput = {
          orderId: firstOrder.customOrderId || orderId,
          createdAt: new Date().toISOString(),
          status: firstOrder.status,
          paymentMethod: payMethod,
          paymentStatus: payMethod === 'wallet' ? 'paid' : payMethod === 'cod' ? 'pending' : firstOrder.paymentStatus,
          buyerName: user?.name ?? 'Guest',
          buyerPhone: phone,
          deliveryAddress: address,
          sellerName: items[0]?.product?.seller?.name ?? '',
          storeName: items[0]?.product?.seller?.storeName ?? 'Local Shop',
          // Real seller phone when the product data has it; never show a
          // fake placeholder number on a receipt the buyer may actually call.
          sellerPhone: sellerPhone || 'See order page for contact details',
          items: items.map((i) => ({ name: i.product.name, price: i.product.price, quantity: i.quantity, image: (i.product.images ?? [])[0] })),
          subtotal, deliveryFee, platformFee, total,
          estimatedTime: eta,
          viewerRole: 'buyer',
        };
        setReceiptData(receipt);
        clearCart(); setPlacing(false); setStep('confirmed');
      }
    } catch (err: any) {
      setPlacing(false);
  ...
  if (step === 'confirmed') {
    return (
      <SafeAreaView style={[s.root, { backgroundColor: colors.background }]} edges={['top']}>
        <Header title="Order Confirmed" onBack={() => router.replace('/(tabs)' as any)} colors={colors} showBack={false} />
        <ScrollView showsVerticalScrollIndicator={false}>
          <LinearGradient colors={[colors.primary, colors.primaryLight]} style={s.successBanner}>
            ...
            <Text style={s.successRef}>Ref: WMK-{currentOrderId.slice(-8).toUpperCase()}</Text>
            <Text style={s.successSub}>
              {payMethod === 'cod'
                ? 'Pay the exact amount to the rider on delivery.'
                : payMethod === 'wallet'
                ? 'Paid from your WimaKit Wallet.'
                : 'Your BNPL instalment plan is active.'}
            </Text>
          </LinearGradient>
          <View style={{ paddingHorizontal: Spacing.lg, paddingTop: Spacing.lg }}>
            {receiptData && <ReceiptView data={receiptData} compact />}
          </View>
            ...
            {Platform.OS !== 'web' && (
              ...
              >
                  ...
                  <PlatformMapUrlTile
                    ...
                    flipY={false}
                  />
                  {coords && <PlatformMapMarker coordinate={coords} pinColor={colors.primary} />}
                </PlatformMap>
                {!coords && (
    ...
    <SafeAreaView style={[s.root, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View key={`cart-seller-group-${sellerId}`} style={{ marginBottom: Spacing.lg }}>
              ...
              {shopItems.map(({ product, quantity }) => ( // Ensure ProductCard is also sleek
                <View key={`cart-row-${(product as any)._id || product.id}`} style={[s.cartRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                  <Image source={{ uri: normalizeImageUri((product.images ?? [])[0]) }} style={s.cartImg} contentFit="cover" />
        ...
        <View style={[s.summaryCard, { backgroundColor: colors.surface, shadowColor: '#000', shadowOpacity: 0.03, shadowRadius: 8, elevation: 1, marginHorizontal: Spacing.lg }]}>
          <Text style={[s.sectionTitle, { color: colors.textPrimary }]}>Order Summary</Text>
          {Object.entries(groupedItems).map(([sellerId, shopItems]) => (
            <View key={`summary-cart-store-${sellerId}`} style={{ marginBottom: Spacing.sm }}>
              <Text style={{ fontSize: FontSize.xs, fontWeight: '700', color: colors.primary, marginBottom: 4 }}>
                {shopItems[0]?.product?.seller?.storeName || 'Local Shop'}
              </Text>
              {shopItems.map(({ product, quantity }) => (

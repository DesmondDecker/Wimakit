    ...
    <SafeAreaView style={[s.root, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: Spacing.lg, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        <View style={[s.summaryCard, { backgroundColor: colors.surface, borderColor: colors.border }, Shadow.sm]}>
          ...
          <View style={[s.statusBadge, { backgroundColor: plan.status === 'overdue' ? colors.error+'20' : plan.status === 'defaulted' ? colors.error+'20' : colors.success+'20' }]}>
            <Text style={{ color: (plan.status === 'overdue' || plan.status === 'defaulted') ? colors.error : colors.success, fontSize: 12, fontWeight: '700' }}>
              {plan.status === 'overdue' ? '⚠ Overdue' : plan.status === 'defaulted' ? '⚠ Defaulted' : plan.status === 'paid' ? '✓ Fully Paid' : plan.status === 'cancelled' ? 'Cancelled' : '✓ Active'}
            </Text>
          </View>
          {plan.status === 'defaulted' && (
            <Text style={{ color: colors.error, fontSize: 12, textAlign: 'center', marginTop: 4 }}>
              This plan was marked as defaulted due to non-payment. Contact support to resolve your outstanding balance.
            </Text>
          )}
        </View>
      ...
      {nextInstalment && !['paid', 'defaulted', 'cancelled'].includes(plan.status) && (

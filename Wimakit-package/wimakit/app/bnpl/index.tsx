...
export default function BnplListScreen() {
  ...
  // Previously 'defaulted' matched neither filter above, so a defaulted
  // plan just silently disappeared from this screen — not shown as active,
  // not shown as completed, nowhere. That was harmless while nothing ever
  // actually set status to 'defaulted' (see tasks/bnplOverdueSweep.js); now
  // that it's reachable, a buyer needs to be able to see it and know why
  // their BNPL access was suspended.
  const defaulted = plans.filter((p: any) => p.status === 'defaulted');
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: Spacing.lg, paddingBottom: 60 }}>
        ...
        {isLoading ? <ActivityIndicator color={colors.primary} style={{ marginTop: 30 }} /> : (
          <>
            ...
            {defaulted.length > 0 && (
              <>
                <Text style={[s.sectionTitle, { color: colors.error, marginTop: Spacing.lg }]}>Defaulted</Text>
                {defaulted.map((plan: any) => (
                  <TouchableOpacity key={plan._id}
                    style={[s.planCard, { backgroundColor: colors.surface, borderColor: colors.error }, Shadow.sm]}
                    onPress={() => router.push(`/bnpl/${plan._id}` as any)}>
                    <View style={s.planHeader}>
                      <View style={[s.planBadge, { backgroundColor: colors.error + '20' }]}>
                        <Text style={[s.planBadgeText, { color: colors.error }]}>⚠ {plan.planType.toUpperCase()}</Text>
                      </View>
                    </View>
                    <Text style={[s.planOrder, { color: colors.textPrimary }]}>Order #{plan.orderId?.customOrderId}</Text>
                    <Text style={[s.planAmount, { color: colors.error }]}>{formatPrice(plan.totalAmount)} — Defaulted</Text>
                    <Text style={[s.planNext, { color: colors.textMuted }]}>Contact support to resolve your outstanding balance.</Text>
                  </TouchableOpacity>
                ))}
              </>
            )}

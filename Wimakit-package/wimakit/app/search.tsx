...
const CatChip = memo(({ item, selected, onPress, colors }: any) => {
  ...
  return (
    <TouchableOpacity
      style={[styles.catChip, { backgroundColor: selected ? colors.primary + '15' : colors.surface, borderWidth: 1, borderColor: selected ? colors.primary : colors.border }]}
      onPress={onPress}
      activeOpacity={0.8}
    >
      {isIconEmoji ? (
        ...
        <MaterialCommunityIcons name={(item.icon as any) ?? 'shape-outline'} size={15} color={selected ? colors.primary : colors.textSecondary} />
      )}
      <Text style={[styles.catChipText, { color: selected ? colors.primary : colors.textSecondary, fontWeight: selected ? '700' : '500' }]}>
        {item.name}
      </Text>
    </TouchableOpacity>
  );
...
export default function SearchScreen() {
  ...
  } = useDebouncedSearch(350, sortId, selectedCat, !!currentUser);
  ...
  const products: Product[] = useMemo(() => {
    ...
    // Flatten all pages of results from useInfiniteQuery (supports both .products and .suggestions)
    let list = data.pages.flatMap((page: any) => page.products ?? page.suggestions ?? []);
    return list;
  }, [data]);
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={[styles.catRow, { backgroundColor: colors.background }]}>
        <FlatList
          ...
          renderItem={({ item }: { item: any }) => {
            ...
            const isAll = catId === 'all-pill';
            const isSelected = isAll ? !selectedCat : selectedCat === catId;
            return (
              <CatChip
                item={item}
                selected={isSelected}
                onPress={() => setSelectedCat(isAll || selectedCat === catId ? null : catId)}
                colors={colors}
              />
            );
          }}
      ...
      <View style={[styles.resultsBar, { borderBottomColor: colors.border + '20' }]}>
        <Text style={[styles.resultsText, { color: colors.textMuted }]}>{
          isLoading || isFetching
            ? 'Searching...'
            : debounced.trim().length > 0 || !!selectedCat
              ? `${products.length} result${products.length !== 1 ? 's' : ''}${debounced.trim() ? ` for "${debounced.trim()}"` : ''}`
              : products.length > 0
                ? `${products.length} items suggested for you`
      ...
      ) : products.length === 0 && (debounced.trim().length > 0 || !!selectedCat) ? (

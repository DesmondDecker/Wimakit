...
import { ordersApi, BASE_URL } from '../../utils/api';
...
export default function OrderTrackingScreen() {
  ...
  const { data: order, isLoading } = useQuery({
    queryKey: ['order', id],
    // Was missing this .then(r => r.data) unwrap — `order` used to be the
    // raw AxiosResponse, so every `o.<field>` read below (customOrderId,
    // status, items, total, seller...) was actually reading off the wrapper
    // `{ data: { success, data: order } }` one level too shallow and
    // silently getting `undefined` everywhere. This makes `order` the
    // parsed backend body `{ success, data: order }`, matching what `o =
    // order.data` below has always assumed it was.
  ...
  const o = order.data;
  const isSellerViewing = !!(user?.id && o.seller?._id && String(o.seller._id) === String(user.id));
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View style={[s.mapContainer, { borderColor: colors.border, marginHorizontal: Spacing.lg }]}>
            ...
            >
              <PlatformMapUrlTile
                ...
                flipY={false}
              />
              <PlatformMapMarker coordinate={{ latitude, longitude }} pinColor={colors.primary} title="Delivery Spot" />
            </PlatformMap>
          </View>
          <Button
        ...
        {/* ── Receipt / Invoice ─────────────────────────────────────────────── */}
        {/* Available for every payment method (COD, wallet, BNPL, mobile
            money) any time the order is viewed — not just once, right after
            checkout. Shown to the buyer as a receipt and to the seller as
            an invoice for the same order. */}
        <View style={{ padding: Spacing.lg }}>
          <ReceiptView
            data={receiptFromOrder(o, isSellerViewing ? 'seller' : 'buyer')}
          />
        </View>

...
export default function AboutScreen() {
  ...
  const supportEmail = settings?.supportEmail || 'support@wimakit.sl';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        <View style={s.section}>
          ...
          <View style={[s.linksCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            {[
              { icon: 'email-outline',   label: supportEmail,     action: () => Linking.openURL(`mailto:${supportEmail}`), color: '#4F46E5' },
              { icon: 'web',             label: 'www.wimakit.com',          action: () => Linking.openURL('https://wimakit.com'), color: '#10B981' },
              { icon: 'phone-outline',   label: supportPhone,           action: () => Linking.openURL(`tel:${supportPhone}`), color: '#F59E0B' },

...
import { communityApi, authApi } from '../../utils/api';
...
export default function CommunityScreen() {
  ...
  const handleBlock = useCallback((post: any) => {
    const authorId = post.author?._id ?? post.author?.id;
    const authorName = post.author?.name ?? 'this user';
    if (!authorId) return;
    Alert.alert('Block User', `Block ${authorName}? You won't see their posts in your feed anymore.`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Block', style: 'destructive', onPress: () => {
          authApi.blockUser(authorId)
            .then(() => {
              Toast.show({ type: 'success', text1: `${authorName} blocked` });
              refetch();
            })
            .catch(() => Toast.show({ type: 'error', text1: 'Could not block user' }));
        },
      },
    ]);
  }, [refetch]);

...
export default function SettingsScreen() {
  ...
  // Settings > Notifications previously rendered three <Switch> rows with a
  // hardcoded `value` prop and no onValueChange at all — tapping them
  // visibly flipped (uncontrolled native switch state resets on re-render)
  // but nothing was ever read from or written to anywhere. This is now
  // backed by a real field on the user (models/User.js notificationPrefs)
  // that createNotification actually checks before sending a live
  // push/socket notification.
  const [notifPrefs, setNotifPrefs] = useState({ orderUpdates: true, promotions: true, messages: true });
  useEffect(() => {
    authApi.getNotificationPrefs().then((r) => setNotifPrefs(r.data.notificationPrefs)).catch(() => {});
  }, []);
  const toggleNotifPref = (key: 'orderUpdates' | 'promotions' | 'messages', value: boolean) => {
    setNotifPrefs((p) => ({ ...p, [key]: value })); // optimistic
    authApi.updateNotificationPrefs({ [key]: value }).catch(() => {
      setNotifPrefs((p) => ({ ...p, [key]: !value })); // rollback on failure
  ...
  const handleLogout = () => {
    Alert.alert('Log Out', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          // logout() was previously fire-and-forget here — router.replace()
          // fired before AsyncStorage was cleared / the server call
          // resolved, racing the navigation against the state update.
          // rider-dashboard.tsx already had the correct await-then-navigate
          // pattern; this brings settings.tsx in line with it. Clearing the
          // query cache prevents the next account to log in on this device
          // from briefly seeing this user's cached wallet/orders/notifications
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <SettingRow icon="truck-fast-outline" label="Order Updates"  colors={colors} right={<Switch value={notifPrefs.orderUpdates} onValueChange={(v) => toggleNotifPref('orderUpdates', v)} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#fff" />} />
          <SettingRow icon="tag-outline" label="Promotions"     colors={colors} right={<Switch value={notifPrefs.promotions} onValueChange={(v) => toggleNotifPref('promotions', v)} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#fff" />} />
          <SettingRow icon="message-text-outline" label="Messages"       colors={colors} right={<Switch value={notifPrefs.messages} onValueChange={(v) => toggleNotifPref('messages', v)} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#fff" />} />
        </View>

        {/* Privacy */}
        <SectionLabel label="Privacy & Security" colors={colors} />
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <SettingRow icon="account-cancel-outline" label="Blocked Users" colors={colors} onPress={() => router.push('/blocked-users' as any)} />

...
import { normalizeImageUri, BASE_URL } from '../../utils/api';
    ...
    >
      ...
      <View style={styles.cardTop}>
        {firstItem?.product?.images?.[0] || firstItem?.image ? (
          <Image
            source={{ uri: normalizeImageUri(firstItem?.product?.images?.[0] ?? firstItem?.image) }}
...
export default function OrdersScreen() {
  ...
  // Was keyed on [user] (the whole object) — a new reference on every
  // auth-store update, including the follow/unfollow call in this same
  // screen — so the socket disconnected and reconnected on every such
  // update instead of only when the logged-in user actually changed.
  // Keying on the stable id means the connection survives unrelated
  // profile/follow updates.

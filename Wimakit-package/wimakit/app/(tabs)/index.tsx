...
import { useAuthStore, useCartStore, useOfflineStore } from '../../store';
import { 
  useFeaturedProducts, 
  useProducts, 
  useCategories, 
...
const CategoryPill = memo(({ item, onPress, colors }: any) => {
  ...
  const isIconEmoji = isEmoji(item.icon);
  return (
    <TouchableOpacity
    ...
    >
      {isIconEmoji ? (
        ...
        <MaterialCommunityIcons name={(item.icon as any) ?? 'shape-outline'} size={16} color={color} />
      )}
...
const DEFAULT_BANNER_COLORS: [string, string] = ['#1A4D1A', '#2D7A2D'];

const normalizeGradientColors = (colors: any, fallback: [string, string] = DEFAULT_BANNER_COLORS): [string, string] => {
  if (Array.isArray(colors)) {
    const first = typeof colors[0] === 'string' && colors[0].trim() ? colors[0] : fallback[0];
    const second = typeof colors[1] === 'string' && colors[1].trim() ? colors[1] : first;
    return [first, second];
  }
...
const FALLBACK_BANNERS = [
  { id: 'fb1', title: 'Welcome to WimaKit', sub: 'Sierra Leone\'s commerce platform', color: DEFAULT_BANNER_COLORS, icon: 'storefront-outline' },
];
...
export default function HomeScreen() {
  ...
  const [activeFeed, setActiveFeed] = useState<'suggested' | 'following' | 'exploring'>(user ? 'suggested' : 'exploring');
  ...
  const BANNER_PLACEMENTS = ['home_banner', 'home'];
  const BANNERS = (adsData && adsData.length > 0)
    ? adsData
        .filter((a: any) => a.status === 'active' && BANNER_PLACEMENTS.includes(a.placement))
        .map((a: any) => ({
          id: a._id,
          title: a.title,
          sub: a.subtitle ?? a.description ?? '',
          color: normalizeGradientColors(a.colors ?? DEFAULT_BANNER_COLORS),
          icon: a.icon ?? 'storefront-outline',
          link: a.link,
        }))
    : FALLBACK_BANNERS;
  ...
  useEffect(() => {
    if (!user && activeFeed === 'suggested') {
      setActiveFeed('exploring');
    }
  }, [user, activeFeed]);
  ...
  const keyExtractor = useCallback((item: any, index: number) => item._id ?? item.id ?? `item-${index}`, []);
        ...
        <View style={styles.feedContainer}>
          ...
          {isFeedLoading ? (
            ...
            <View style={styles.emptyFeed}>
              <Text style={{ fontSize: 48 }}>{!isOnline ? '📡' : activeFeed === 'following' ? '👥' : '🔎'}</Text>
              ...
              <Text style={[styles.emptyFeedSub, { color: colors.textMuted }]}>
                {!isOnline
                  ? "Can't load new products right now — check your connection and pull down to try again."
                  : activeFeed === 'following' 
                    ? 'Follow your favorite stores to see their latest products here!' 
                    : 'Check back later for more updates.'}
              </Text>
              {isOnline && activeFeed === 'following' && (

...
const CategoryTile = memo(({ cat, onPress, colors }: any) => {
  ...
  const isIconEmoji = isEmoji(cat.icon);
  return (
    <TouchableOpacity
    ...
    >
      {isIconEmoji ? (
        ...
        <MaterialCommunityIcons name={(cat.icon as any) ?? 'shape-outline'} size={26} color={color} />
      )}
...
export default function ExploreScreen() {
  ...
  // "Trending Now" below only ever showed products an admin had separately
  // marked isFeatured — approving a seller's product doesn't set that flag,
  // so an ordinary newly-approved product had no section on this whole
  // screen that would ever surface it. This pulls straight from the same
  // approved-products listing Home/Search use, so anything that clears
  // moderation shows up here too, featured or not.
  const { data: newArrivalsData, isLoading: newArrivalsLoading } = useProducts({ sort: '-createdAt', limit: 12 });
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        ...
        <View style={styles.section}>
          ...
          {catsLoading ? (
            ...
            <FlatList
              ...
              numColumns={4}
              scrollEnabled={false}
              keyExtractor={(item: any, index: number) => item._id ?? item.id ?? item.slug ?? `cat-${index}`}
        ...
        >
          ...
          >
            ...
            <MaterialCommunityIcons name={"star-four-points-outline" as any} size={64} color="#fff" />
          </LinearGradient>
        </MotiView>
        ...
        <View style={styles.section}>
          ...
          <FlatList
            ...
            keyExtractor={(item: any, index: number) => item._id ?? item.id ?? item.profileSlug ?? `seller-${index}`}
        ...
        <View style={styles.section}>
          ...
          {featuredLoading ? (
            ...
            <FlatList
              ...
              numColumns={2}
              scrollEnabled={false}
              keyExtractor={(item: any, index: number) => item._id ?? item.id ?? `feat-${index}`}
        ...
        {/* New arrivals — every approved product, newest first (not just featured) */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>🆕 New Arrivals</Text>
            <TouchableOpacity onPress={() => router.push('/search?sort=-createdAt' as any)}>
              <Text style={[styles.seeAll, { color: colors.primary }]}>See all →</Text>
            </TouchableOpacity>
          </View>
          {newArrivalsLoading ? (
            ...
            <Text style={{ color: colors.textMuted, paddingHorizontal: Spacing.lg, fontSize: FontSize.sm }}>
            ...
            <FlatList
              ...
              numColumns={2}
              scrollEnabled={false}
              keyExtractor={(item: any, index: number) => item._id ?? item.id ?? `new-${index}`}
              renderItem={renderProduct}
              contentContainerStyle={{ paddingHorizontal: Spacing.lg }}
              columnWrapperStyle={{ gap: Spacing.md }}
              initialNumToRender={4}

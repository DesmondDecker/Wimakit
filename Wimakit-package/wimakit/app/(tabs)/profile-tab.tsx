  ...
  const handleLogout = useCallback(() => {
    Alert.alert('Log Out', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        // Same fix as settings.tsx and the pattern rider-dashboard.tsx
        // already used: await the store's logout() (AsyncStorage clear +
        // best-effort server call) before navigating, and clear the React
        // Query cache so a different account logging in on this device
        // doesn't briefly see this user's cached data.
        onPress: () => {
          logout().then(() => {
            queryClient.clear();

...
import {
  ...
  useApproveKyc, useReviewLoan, useApproveProduct, useAdminDeleteProduct, useApprovePayout, useRejectPayout, useBanUser, useUnbanUser,
  useSetBnplEligibility, useSetLoanEligibility, useForgiveBnplDefault, useCancelBnplPlan, useWaiveBnplLateFees,
  useAdminLegalPages, useUpdateLegalPage, useSiteSettings, useUpdateSiteSettings,
  useResolveDispute, useForceCancelOrder, useForceRefundOrder,
  useResetUserPassword, useUserActivity,
} from '../../hooks/useApi';
...
type ModuleId = 'dashboard'|'users'|'products'|'sellers'|'riders'|'kyc'|'orders'|'disputes'|'payouts'|'bnpl'|'loans'|'ads'|'community'|'broadcast'|'health'|'escrow'|'delivery'|'financial'|'content';
...
const MODULES: { id: ModuleId; label: string; icon: string; color: string }[] = [
  ...
  { id:'content',   label:'Content',    icon:'file-document-edit-outline', color:'#0D9488' },
];
  ...
  return (
    <SafeAreaView style={[s.root, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: Spacing.lg, paddingBottom: 60 }}>
        ...
        {active === 'content'   && <ContentModule   colors={colors} />}
...
function UsersModule({ colors }: any) {
  ...
  const { data: activity, isLoading: activityLoading } = useUserActivity(activityTarget?._id ?? activityTarget?.id);
  ...
  const handleResetPassword = () => {
    ...
    resetMut.mutate({ id: resetTarget._id ?? resetTarget.id, newPassword }, {
      onSuccess: () => { Toast.show({ type: 'success', text1: `Password reset for ${resetTarget.name}` }); setResetTarget(null); setNewPassword(''); },
      onError: (e: any) => Toast.show({ type: 'error', text1: e?.response?.data?.message ?? 'Reset failed' }),
    });
  };
  ...
  return (
    <View>
        ...
        users.map((u: any) => (
          <View key={u._id ?? u.id} style={[s.userCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <View style={{ gap: 6 }}>
              ...
              <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.surfaceAlt ?? colors.surface, borderWidth: 1, borderColor: colors.border }]}
                onPress={() => { setNewPassword(''); setResetTarget(u); }}>
                <Text style={{ color: colors.textPrimary, fontSize: 11, fontWeight: '700' }}>Reset Password</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.surfaceAlt ?? colors.surface, borderWidth: 1, borderColor: colors.border }]}
                onPress={() => setActivityTarget(u)}>
                <Text style={{ color: colors.textPrimary, fontSize: 11, fontWeight: '700' }}>View Activity</Text>
              </TouchableOpacity>
            </View>
          </View>
        ))
      ...
      <Modal visible={!!resetTarget} transparent animationType="fade" onRequestClose={() => setResetTarget(null)}>
        <View style={s.modalOverlay}>
          <View style={[s.modalCard, { backgroundColor: colors.surface }]}>
            <Text style={[s.modalTitle, { color: colors.textPrimary }]}>Reset password — {resetTarget?.name}</Text>
            <Text style={[s.userMeta, { color: colors.textMuted, marginBottom: 10 }]}>
              The user will be notified that an admin changed their password.
            </Text>
            <TextInput
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 12 }}>
              <TouchableOpacity style={[s.smallBtn, { flex: 1, alignItems: 'center', backgroundColor: colors.surfaceAlt ?? colors.surface, borderWidth: 1, borderColor: colors.border }]}
                onPress={() => setResetTarget(null)}>
                <Text style={{ color: colors.textPrimary, fontWeight: '700' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity disabled={resetMut.isPending} style={[s.smallBtn, { flex: 1, alignItems: 'center', backgroundColor: colors.primary }]}
                onPress={handleResetPassword}>
                {resetMut.isPending ? <ActivityIndicator color="#fff" size="small" /> : <Text style={{ color: '#fff', fontWeight: '700' }}>Reset</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={!!activityTarget} transparent animationType="fade" onRequestClose={() => setActivityTarget(null)}>
        <View style={s.modalOverlay}>
          <View style={[s.modalCard, { backgroundColor: colors.surface, maxHeight: '80%' }]}>
            <Text style={[s.modalTitle, { color: colors.textPrimary }]}>{activityTarget?.name}'s activity</Text>
            {activityLoading ? <ActivityIndicator color={colors.primary} style={{ marginTop: 16 }} /> : (
              <ScrollView style={{ marginTop: 10 }}>
                <Text style={{ color: colors.textPrimary, fontWeight: '800', marginBottom: 6 }}>
                  Posts ({activity?.posts?.length ?? 0})
                </Text>
                {(activity?.posts ?? []).length === 0 && <Text style={{ color: colors.textMuted, fontSize: 12, marginBottom: 12 }}>No posts</Text>}
                {(activity?.posts ?? []).map((p: any) => (
                  <View key={p._id} style={[s.userCard, { backgroundColor: colors.surfaceAlt ?? colors.surface, borderColor: colors.border, marginBottom: 8 }]}>
                    <Text style={{ color: colors.textPrimary, fontSize: 12 }} numberOfLines={2}>{p.content || '(image post)'}</Text>
                    <Text style={{ color: colors.textMuted, fontSize: 10, marginTop: 4 }}>{timeAgo(p.createdAt)}</Text>
                  </View>
                ))}

                <Text style={{ color: colors.textPrimary, fontWeight: '800', marginTop: 10, marginBottom: 6 }}>
                  Sales ({activity?.sales?.length ?? 0})
                </Text>
                {(activity?.sales ?? []).length === 0 && <Text style={{ color: colors.textMuted, fontSize: 12, marginBottom: 12 }}>No sales</Text>}
                {(activity?.sales ?? []).map((o: any) => (
                  <View key={o._id} style={[s.userCard, { backgroundColor: colors.surfaceAlt ?? colors.surface, borderColor: colors.border, marginBottom: 8 }]}>
                    <Text style={{ color: colors.textPrimary, fontSize: 12, fontWeight: '700' }}>#{o.customOrderId ?? o._id} · {formatPrice(o.total ?? 0)}</Text>
                    <Text style={{ color: colors.textMuted, fontSize: 10, marginTop: 2 }}>{o.status} · {timeAgo(o.createdAt)}</Text>
                  </View>
                ))}

                <Text style={{ color: colors.textPrimary, fontWeight: '800', marginTop: 10, marginBottom: 6 }}>
                  Purchases ({activity?.purchases?.length ?? 0})
                </Text>
                {(activity?.purchases ?? []).length === 0 && <Text style={{ color: colors.textMuted, fontSize: 12, marginBottom: 12 }}>No purchases</Text>}
                {(activity?.purchases ?? []).map((o: any) => (
                  <View key={o._id} style={[s.userCard, { backgroundColor: colors.surfaceAlt ?? colors.surface, borderColor: colors.border, marginBottom: 8 }]}>
                    <Text style={{ color: colors.textPrimary, fontSize: 12, fontWeight: '700' }}>#{o.customOrderId ?? o._id} · {formatPrice(o.total ?? 0)}</Text>
                    <Text style={{ color: colors.textMuted, fontSize: 10, marginTop: 2 }}>{o.status} · {timeAgo(o.createdAt)}</Text>
                  </View>
            ...
            <TouchableOpacity style={[s.smallBtn, { alignItems: 'center', marginTop: 12, backgroundColor: colors.surfaceAlt ?? colors.surface, borderWidth: 1, borderColor: colors.border }]}
              onPress={() => setActivityTarget(null)}>
...
function ProductsModule({ colors }: any) {
  ...
  // Admin-side counterpart to the seller's own "delete my product" action —
  // lets moderation remove an already-approved listing (counterfeit, banned
  // item discovered post-approval, seller request, etc.) without having to
  // impersonate the seller. Hits the same soft-delete endpoint sellers use;
  // the backend already authorises admins on it.
  const handleDelete = (p: any) => {
    Alert.alert(
      'Delete Product',
      `Remove "${p.name}" from WimaKit? This cannot be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Delete',
          ...
          onPress: () => {
            deleteMut.mutate(p._id, {
              onSuccess: () => { Toast.show({ type: 'success', text1: 'Product deleted' }); refetch(); },
              onError: () => Toast.show({ type: 'error', text1: 'Failed to delete product' }),
            });
          },
        },
      ]
  ...
  const handleTrend = (p: any) => {
    adminApi.trendProduct(p._id, true).then(() => { Toast.show({ type:'success', text1:'Product set as trending 🔥' }); refetch(); });
  };

  // Products previously had no way to un-trend at all — only a one-way
  // "Set Trending" button, unlike the Sellers panel which at least *tried*
  // to have a remove action (see SellersModule.tsx fix). Adding the
  // matching remove action here for parity, now that the backend endpoint
  // actually supports isTrending: false.
  const handleUntrend = (p: any) => {
    adminApi.trendProduct(p._id, false).then(() => { Toast.show({ type:'success', text1:'Trending removed' }); refetch(); });
  };
              ...
              {p.status === 'approved' && (
                <View style={{ flexDirection: 'row', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  {!p.isTrending ? (
                    ...
                    <TouchableOpacity style={[s.statusBadge,{backgroundColor:'#F59E0B20'}]} onPress={() => handleUntrend(p)}>
                      <Text style={{color:'#F59E0B',fontSize:10,fontWeight:'700'}}>🔥 Trending · Tap to remove</Text>
                    </TouchableOpacity>
                  )}
                  <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.error+'20' }]} onPress={() => handleDelete(p)}>
                    <Text style={{ color: colors.error, fontSize: 11, fontWeight: '700' }}>🗑️ Delete</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
...
function BnplModule({ colors }: any) {
  ...
  const { data, isLoading, refetch } = useAdminBnpl({ status: filter === 'all' ? undefined : filter });
  const forgiveMut = useForgiveBnplDefault();
  const cancelMut  = useCancelBnplPlan();
  const waiveMut   = useWaiveBnplLateFees();
  const plans = data?.plans ?? [];

  const handleForgive = (p: any) => {
    Alert.alert('Forgive default', `Reinstate ${p.userId?.name}'s plan and restore their BNPL access?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Forgive', onPress: () => forgiveMut.mutate({ id: p._id }, {
        onSuccess: () => { Toast.show({ type:'success', text1:'Default forgiven, plan reinstated' }); refetch(); },
        onError: (e: any) => Toast.show({ type:'error', text1: e?.response?.data?.message ?? 'Failed' }),
      }) },
    ]);
  };
  const handleCancel = (p: any) => {
    Alert.alert('Cancel plan', `Cancel this plan and clear ${p.userId?.name}'s remaining balance? Use this for refunded/returned orders.`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Cancel Plan', style: 'destructive', onPress: () => cancelMut.mutate({ id: p._id, reason: 'Admin cancellation' }, {
        onSuccess: () => { Toast.show({ type:'success', text1:'Plan cancelled' }); refetch(); },
        onError: (e: any) => Toast.show({ type:'error', text1: e?.response?.data?.message ?? 'Failed' }),
      }) },
    ]);
  };
  const handleWaive = (p: any) => {
    Alert.alert('Waive late fees', `Waive ${formatPrice(p.totalLateFees)} in late fees for ${p.userId?.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Waive', onPress: () => waiveMut.mutate({ id: p._id }, {
        onSuccess: () => { Toast.show({ type:'success', text1:'Late fees waived' }); refetch(); },
        onError: (e: any) => Toast.show({ type:'error', text1: e?.response?.data?.message ?? 'Failed' }),
      }) },
    ]);
  };
  ...
  return (
    <View>
      <View style={s.filterRow}>
        {['overdue','defaulted','active','paid','cancelled','all'].map(f => (
          <TouchableOpacity key={f} style={[s.filterChip, { backgroundColor: filter === f ? colors.primary : colors.surfaceAlt ?? colors.surface, borderColor: colors.border }]} onPress={() => setFilter(f)}>
            <Text style={{ color: filter === f ? '#fff' : colors.textMuted, fontSize: 11, fontWeight: '700' }}>{f}</Text>
          </TouchableOpacity>
        ))}
      </View>
      {isLoading ? <ActivityIndicator color={colors.primary} style={{ marginTop: 20 }} /> : plans.length === 0 ? (
        <EmptyState icon="calendar-month" text="No BNPL plans here" colors={colors} />
      ) : (
        plans.map((p: any) => (
          <View key={p._id} style={[s.disputeCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={{ flex: 1 }}>
              ...
              {p.totalLateFees > 0 && (
                <Text style={[s.userMeta, { color: colors.error }]}>Late fees: {formatPrice(p.totalLateFees)}</Text>
              )}
              <View style={[s.statusBadge, { alignSelf: 'flex-start', marginTop: 4, backgroundColor: p.status === 'active' ? colors.success+'20' : (p.status === 'overdue' || p.status === 'defaulted') ? colors.error+'20' : colors.surfaceAlt }]}>
                <Text style={{ color: p.status === 'active' ? colors.success : (p.status === 'overdue' || p.status === 'defaulted') ? colors.error : colors.textMuted, fontSize: 10, fontWeight:'700' }}>{p.status}</Text>
              </View>
              {/* Real actions — previously this module could only display a
                  status badge, with no way for an admin to actually do
                  anything about an overdue or defaulted plan. */}
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                {p.status === 'defaulted' && (
                  <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.success+'20' }]} onPress={() => handleForgive(p)}>
                    <Text style={{ color: colors.success, fontSize: 11, fontWeight: '700' }}>Forgive Default</Text>
                  </TouchableOpacity>
                )}
                {p.totalLateFees > 0 && !['paid','cancelled'].includes(p.status) && (
                  <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.primaryMuted }]} onPress={() => handleWaive(p)}>
                    <Text style={{ color: colors.primary, fontSize: 11, fontWeight: '700' }}>Waive Late Fees</Text>
                  </TouchableOpacity>
                )}
                {!['paid','cancelled'].includes(p.status) && (
                  <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.error+'20' }]} onPress={() => handleCancel(p)}>
                    <Text style={{ color: colors.error, fontSize: 11, fontWeight: '700' }}>Cancel Plan</Text>
                  </TouchableOpacity>
                )}
              </View>
            </View>
...
// ─── Content Module ───────────────────────────────────────────────────────────
// Legal pages (Privacy, Terms, Cookies, Help) and support contact info were
// previously hardcoded directly in the app bundle — no admin editing
// existed at all, so fixing a typo or updating the support number meant a
// full app store release. This is real editing, not a preview: saves go
// straight to the same GET /api/content/legal/:slug and
// GET /api/content/settings the app itself reads from.
function ContentModule({ colors }: any) {
  const { data: pages, isLoading, refetch } = useAdminLegalPages();
  const { data: settings, refetch: refetchSettings } = useSiteSettings();
  const updatePage = useUpdateLegalPage();
  const updateSettings = useUpdateSiteSettings();

  const [editingSlug, setEditingSlug] = useState<string | null>(null);
  const [draft, setDraft] = useState<{ title: string; icon: string; sections: { heading: string; body: string }[] } | null>(null);

  const [settingsDraft, setSettingsDraft] = useState({ supportPhone: '', supportEmail: '', supportWhatsApp: '' });
  useEffect(() => {
    if (settings) setSettingsDraft({ supportPhone: settings.supportPhone ?? '', supportEmail: settings.supportEmail ?? '', supportWhatsApp: settings.supportWhatsApp ?? '' });
  }, [settings]);

  const openEditor = (page: any) => {
    setEditingSlug(page.slug);
    setDraft({ title: page.title, icon: page.icon, sections: page.sections.map((s: any) => ({ ...s })) });
  };

  const saveSettings = () => {
    updateSettings.mutate(settingsDraft, {
      onSuccess: () => { Toast.show({ type: 'success', text1: 'Support contact info updated' }); refetchSettings(); },
  ...
  const savePage = () => {
    ...
    if (draft.sections.some((s) => !s.heading.trim() || !s.body.trim())) {
      Toast.show({ type: 'error', text1: 'Every section needs a heading and body' });
      return;
    }
    updatePage.mutate({ slug: editingSlug, ...draft }, {
      onSuccess: () => { Toast.show({ type: 'success', text1: 'Page updated' }); setEditingSlug(null); refetch(); },
      onError: () => Toast.show({ type: 'error', text1: 'Failed to save' }),
    });
  };
  ...
  return (
    <View>
      {/* Support contact */}
      <Text style={{ color: colors.textPrimary, fontWeight: '800', marginBottom: 6 }}>Support Contact</Text>
      <View style={[s.disputeCard, { backgroundColor: colors.surface, borderColor: colors.border, flexDirection: 'column', alignItems: 'stretch', gap: 8 }]}>
        <Text style={{ color: colors.textMuted, fontSize: 11 }}>Support phone (used on Help & Support and About screens)</Text>
        <TextInput value={settingsDraft.supportPhone} onChangeText={(t) => setSettingsDraft((d) => ({ ...d, supportPhone: t }))}
          style={[s.modalInput, { color: colors.textPrimary, borderColor: colors.border }]} placeholder="+23276000000" placeholderTextColor={colors.textMuted} />
        <Text style={{ color: colors.textMuted, fontSize: 11 }}>Support email</Text>
        <TextInput value={settingsDraft.supportEmail} onChangeText={(t) => setSettingsDraft((d) => ({ ...d, supportEmail: t }))}
          style={[s.modalInput, { color: colors.textPrimary, borderColor: colors.border }]} placeholder="support@wimakit.sl" placeholderTextColor={colors.textMuted} autoCapitalize="none" />
        <Text style={{ color: colors.textMuted, fontSize: 11 }}>Support WhatsApp</Text>
        <TextInput value={settingsDraft.supportWhatsApp} onChangeText={(t) => setSettingsDraft((d) => ({ ...d, supportWhatsApp: t }))}
          style={[s.modalInput, { color: colors.textPrimary, borderColor: colors.border }]} placeholder="+23276000000" placeholderTextColor={colors.textMuted} />
        <TouchableOpacity style={[s.smallBtn, { backgroundColor: colors.primary, alignSelf: 'flex-start', marginTop: 4 }]} onPress={saveSettings}>
          <Text style={{ color: '#fff', fontSize: 12, fontWeight: '700' }}>Save Contact Info</Text>
        </TouchableOpacity>
      </View>

      {/* Legal pages list */}
      <Text style={[{ color: colors.textPrimary, fontWeight: '800', marginBottom: 6 }, { marginTop: 20 }]}>Legal & Help Pages</Text>
      {isLoading ? <ActivityIndicator color={colors.primary} style={{ marginTop: 20 }} /> : (
        (pages ?? []).map((page: any) => (
          <TouchableOpacity key={page.slug} style={[s.disputeCard, { backgroundColor: colors.surface, borderColor: colors.border }]} onPress={() => openEditor(page)}>
            <View style={{ flex: 1 }}>
              <Text style={[s.userName, { color: colors.textPrimary }]}>{page.title}</Text>
              <Text style={[s.userMeta, { color: colors.textMuted }]}>/{page.slug} · {page.sections.length} section{page.sections.length === 1 ? '' : 's'} · v{page.version}</Text>
            </View>
      ...
      {/* Editor modal */}
      <Modal visible={!!editingSlug} animationType="slide" onRequestClose={() => setEditingSlug(null)}>
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: Spacing.lg }}>
            <TouchableOpacity onPress={() => setEditingSlug(null)}><Text style={{ color: colors.textMuted }}>Cancel</Text></TouchableOpacity>
            <Text style={{ color: colors.textPrimary, fontWeight: '700', fontSize: 16 }}>Edit {editingSlug}</Text>
            <TouchableOpacity onPress={savePage} disabled={updatePage.isPending}>
              <Text style={{ color: colors.primary, fontWeight: '700' }}>{updatePage.isPending ? 'Saving…' : 'Save'}</Text>
            </TouchableOpacity>
          </View>
          <ScrollView contentContainerStyle={{ padding: Spacing.lg, gap: 12 }}>
            {draft && (
              <>
                ...
                <TextInput value={draft.title} onChangeText={(t) => setDraft((d) => d && { ...d, title: t })}
                  style={[s.modalInput, { color: colors.textPrimary, borderColor: colors.border }]} />
                {draft.sections.map((sec, i) => (
                  <View key={i} style={{ borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 10, gap: 6 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Text style={{ color: colors.textMuted, fontSize: 11 }}>Section {i + 1}</Text>
                      <TouchableOpacity onPress={() => setDraft((d) => d && { ...d, sections: d.sections.filter((_, j) => j !== i) })}>
                        <MaterialCommunityIcons name="trash-can-outline" size={16} color={colors.error} />
                      </TouchableOpacity>
                    </View>
                    <TextInput value={sec.heading} onChangeText={(t) => setDraft((d) => d && { ...d, sections: d.sections.map((x, j) => j === i ? { ...x, heading: t } : x) })}
                      placeholder="Heading" placeholderTextColor={colors.textMuted}
                      style={[s.modalInput, { color: colors.textPrimary, borderColor: colors.border }]} />
                    <TextInput value={sec.body} onChangeText={(t) => setDraft((d) => d && { ...d, sections: d.sections.map((x, j) => j === i ? { ...x, body: t } : x) })}
                      placeholder="Body" placeholderTextColor={colors.textMuted} multiline numberOfLines={4}
                      style={[s.modalInput, { color: colors.textPrimary, borderColor: colors.border, minHeight: 80, textAlignVertical: 'top' }]} />
                  </View>
                ))}
                <TouchableOpacity
                  style={[s.smallBtn, { backgroundColor: colors.primaryMuted, alignSelf: 'flex-start' }]}
                  onPress={() => setDraft((d) => d && { ...d, sections: [...d.sections, { heading: '', body: '' }] })}
                >
                  <Text style={{ color: colors.primary, fontSize: 12, fontWeight: '700' }}>+ Add Section</Text>
                </TouchableOpacity>
              </>
            )}
          </ScrollView>
...
function CreateAdForm({ colors, onCreated }: any) {
  ...
  const submit = () => {
    ...
    setSaving(true);
    // The Ad schema has separate `subtitle` and `description` fields, but
    // PostCard's feed-ad card only ever reads `ad.description` (see
    // components/community/PostCard.tsx) — so whatever the admin typed
    // here was saved but never rendered anywhere. Sending it as both
    // keeps the field this form's label promises ("Subtitle /
    // description") actually visible on the card.
    adminApi.createAd({ title: title.trim(), subtitle: subtitle.trim() || undefined, description: subtitle.trim() || undefined, targetUrl: targetUrl.trim() || undefined, placement, audienceRole })
      .then(() => {
  ...
  return (
    <SectionCard title="Create Ad" colors={colors}>
      ...
      {/* "feed" only shows woven into the Community feed (roughly every 5
          posts) — pick "home_banner" if you want it visible immediately
          on the Home screen's hero banner carousel instead. */}
      <Text style={{ color: colors.textMuted, fontSize: 11, marginBottom: 8 }}>
        {placement === 'feed'
          ? 'Shows inside the Community feed, woven in roughly every 5 posts.'
          : placement === 'home_banner' || placement === 'home'
            ? 'Shows immediately in the Home screen banner carousel.'
            : `Shows on the ${placement.replace('_', ' ')} screen.`}
      </Text>

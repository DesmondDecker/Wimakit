...
import { prepareImagesForUpload } from '../../utils/imagePrep';
...
export default function AddProductScreen() {
  ...
  const pickImages = useCallback(async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ['images'], // MediaTypeOptions is deprecated as of expo-image-picker v17 (SDK 54)
    ...
    if (!result.canceled) {
      ...
      try {
        // Resize/re-compress on-device before these ever touch the upload —
        // see utils/imagePrep.ts for why this is the actual fix for "upload
        // is slow": it's the photo dimensions, not the API call.
        const prepared = await prepareImagesForUpload(result.assets);
        setImages(p => [...p, ...prepared].slice(0, 8));
      } finally {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    createProduct.mutate({ formData: fd, onUploadProgress: setUploadPct }, {
      onSuccess: () => {
        Toast.show({ type: 'success', text1: '🎉 Product submitted!', text2: 'Admin will review it shortly.' });
        router.back();
      },
      onError: (e: any) => { setUploadPct(0); Toast.show({ type: 'error', text1: e?.response?.data?.message ?? 'Failed to submit' }); },
    });
      ...
      case 'basic': return (
        <View style={s.stepContent}>
          ...
          <View style={s.imagesGrid}>
            ...
            {images.length < 8 && (
              <TouchableOpacity
                style={[s.addImageBtn, { backgroundColor: colors.surfaceAlt ?? colors.surface, borderColor: colors.border }]}
                onPress={pickImages}
              ...
              >
                {preparingImages ? (
                  <>
                    ...
                    <Text style={[s.addImageSub, { color: colors.textMuted, marginTop: 6 }]}>Preparing…</Text>
                  </>
                ) : (
                  <>
            ...
            <TouchableOpacity
              ...
              {createProduct.isPending ? (
                <>
                  ...
                  <Text style={s.submitBtnText}>{uploadPct > 0 ? `Uploading… ${uploadPct}%` : 'Submitting…'}</Text>
                </>
              ) : (
                <>

...
import { useSellerProducts, useDeleteProduct, useWallet, useSellerOrders } from '../../hooks/useApi';
...
function ProductRow({ product: p, onDelete, onEdit, colors }: any) {
  ...
  return (
    <View style={[pr.row, { backgroundColor: colors.surface, borderColor: colors.border }, Shadow.sm]}>
      <Image source={{ uri: normalizeImageUri(p.images?.[0]) }} style={pr.img} contentFit="cover" />
      <View style={{ flex:1 }}>
...
export default function SellerDashboard() {
  ...
  // Sellers previously had no way at all to view their orders, invoices, or
  // receipts anywhere in the app — useSellerOrders() already existed in
  // hooks/useApi.ts but nothing rendered it. This wires it into the one
  // screen sellers actually land on.
  const { data: ordersData, refetch: refetchOrders } = useSellerOrders({ limit: 5 });
            ...
            <View style={{ marginTop: Spacing.lg }}>
              <View style={s.sectionHeader}>
                ...
                <TouchableOpacity onPress={() => router.push('/(tabs)/orders' as any)}>
                  <Text style={{ color: colors.primary, fontWeight: '700', fontSize: FontSize.sm }}>See all →</Text>
                </TouchableOpacity>
              </View>
              <View style={{ paddingHorizontal: Spacing.lg, gap: Spacing.sm }}>
                {recentOrders.map((ord: any) => (
                  <TouchableOpacity
                    key={ord._id}
                    style={[s.orderRow, { backgroundColor: colors.surface, borderColor: colors.border }]}
                    onPress={() => router.push(`/order/${ord.customOrderId ?? ord._id}` as any)}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={[s.orderRowId, { color: colors.textPrimary }]}>#{ord.customOrderId}</Text>
                      <Text style={[s.orderRowStatus, { color: colors.textMuted }]}>
                        {String(ord.status ?? 'pending').replace(/_/g, ' ')} · {new Date(ord.createdAt).toLocaleDateString()}
                      </Text>
                    </View>
                    <Text style={[s.orderRowTotal, { color: colors.primary }]}>{formatPrice(ord.total)}</Text>
...
const s = StyleSheet.create({
  ...
  // Recent orders (Receipt/Invoice entry point)
  orderRow:        { flexDirection:'row', alignItems:'center', gap: Spacing.sm, padding: Spacing.md, borderRadius: Radius.lg, borderWidth:1 },
  orderRowId:      { fontSize:13, fontWeight:'800' },
  orderRowStatus:  { fontSize:11, marginTop:2, textTransform:'capitalize' },
  orderRowTotal:   { fontSize:13, fontWeight:'900', marginRight:4 },

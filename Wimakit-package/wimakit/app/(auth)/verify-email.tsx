 ...
 * verify-email.tsx
 * ─────────────────
 * There is no numeric OTP anywhere in this flow — the verification token
 * is a 64-character random hex string (see makeToken() in routes/auth.js),
 * far too long to hand-type or split across 6 boxes. This screen has
 * exactly one real entry point:
 *
 *   Deep link:  wimakit://verify-email?token=<rawToken>
 *     → auto-verifies on mount, shows success/error
 *
 * If the deep link doesn't fire (email client strips it, or the person
 * opens the email on a different device than the app is installed on),
 * the only fallback is the resend button below — there is currently no
 * manual token-entry field. A previous version of this comment described
 * "a 6-box OTP-style input for token" as a second entry point; that was
 * never actually implemented (no TextInput exists in the idle-state render
 * at all), so it's been removed here rather than left to mislead the next
 * person who reads this file. If that gap needs closing, it should be a
 * short random code (e.g. 6 digits via crypto.randomInt, stored hashed,
 * single-use, short expiry) generated specifically for manual entry —
 * not a truncation of the 64-char link token, which would leak most of a
 * token whose whole security property depends on being unguessable.
 */
